...
module.exports.index = async(ctx) => {
  ...
  const disasterEventData = await getEvents("disasters", "3,4,5");
  //console.log("get severe-weather");
  //const severeWeatherEventData = await getEvents("severe-weather", 5);
  console.log("get terror");
  const terrorEventData = await getEvents("terror", "3,4,5");

  //const eventCollection = [disasterEventData, terrorEventData, severeWeatherEventData];
  const eventCollection = [disasterEventData, terrorEventData];
...
function getEvents(category, rank) {
  ...
  return client.events.search({
    ...
    state: "active",
    country: "US",
    "start.gte": moment().subtract(1, "days").format("YYYY-MM-DD")
  });
}
